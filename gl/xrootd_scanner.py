...
try:
    from gfal_client import GFALClient
except ImportError:
    GFALClient = None
...
class Prescanner(Primitive):
    ...
    class PrescannerTask(Task):
        ...
        def __init__(self, server, server_root, is_redirector, root, timeout, use_gfal):
            ...
            self.Failed = False
            self.Error = None
            self.UseGFAL = use_gfal

        def run(self):
            if self.UseGFAL:
                self.Client = GFALClient(self.Server, self.ServerRoot,
                        timeout=self.Timeout, name=f"GFALClient({self.Root})")
    ...
    def __init__(self, server, server_root, is_redirector, roots, timeout, max_scanners, use_gfal=False):
        Primitive.__init__(self)
        self.Good = []              # [client, ...]
        self.Failed = {}            # {root: error}
        self.Queue = TaskQueue(max_scanners, stagger=0.5, delegate=self,
            tasks = [self.PrescannerTask(server, server_root, is_redirector, root, timeout, use_gfal) for root in roots]
        )
...
class ScannerMaster(PyThread):
    ...
    def run(self):
        #
        # scan Root non-recursively first, if failed, return immediately
        #
...
python xrootd_scanner.py [options] <rse>
    ...
    -T                          - turn tracing on
    -g                          - use gfal-ls instead of xrootd native commands
"""
...
def main():
    ...
    t0 = time.time()
    opts, args = getopt.getopt(sys.argv[1:], "t:m:o:R:n:c:vqM:s:S:zkxe:r:E:Tg")
    opts = dict(opts)

    if len(args) != 1 or not "-c" in opts:
        print("Version:", Version)
        print(Usage)
        print(f'User-provided opts: {opts}')
        sys.exit(2)
    ...
    use_gfal = "-g" in opts
    if use_gfal and GFALClient is None:
        print("Error: gfal_client module not available (gfal-ls/HTTPS mode requires gfal_client.py)")
        sys.exit(2)

    quiet = "-q" in opts
    display_progress = not quiet and "-v" in opts
    print(f"display_progress={display_progress}")
    max_files = int(opts.get("-M", 0)) or None

    if use_gfal:
        recursive_threshold = 999999
    else:
    ...
    if compute_empty_dirs and "-E" in opts:
        ...
        print(f"=== modulo={modulo} - day_number={day_number} mod={day_number%modulo} | rse_hash={rse_hash} mod={rse_hash%modulo}")
    ...
    my_stats = {
        "rse":rse,
        "scanner":{
            "type":"gfal" if use_gfal else "xrootd",
            "version":Version
        },
    ...
    t0 = time.time()
    good_roots, failed_roots = Prescanner(server, server_root, config.ServerIsRedirector, config.RootList, config.ScannerTimeout, max_scanners, use_gfal=use_gfal).run()
    print(f" good_roots={good_roots} failed_roots={failed_roots}")
    t1 = time.time()
    ...
    for root, error in failed_roots.items():
        ...
        print(f" @sp1: expected={expected} root={root}, failed={failed}, error={error}")
        failed = failed or expected

    print(f"@sp2: failed={failed}")
    all_roots_failed = False
    total_files = 0
    print(f" @sp3: failed={failed}, all_roots_failed={all_roots_failed}")
    if not failed:
    ...
    print(f" @sp4: failed={failed} total_files={total_files} all_roots_failed={all_roots_failed}")
    if failed or all_roots_failed or total_files == 0:
        print(f" @sp4a: setting status=failed")
        my_stats["status"] = "failed"
    else:
        print(f" @sp4a: setting status=done")
    ...
    if failed or all_roots_failed:
        ...
        if all_roots_failed is not None:
            print(f" | all_roots_failed={all_roots_failed}")
        else:
            print(f" all_roots_failed: None")
        sys.exit(2)
    else:
        sys.exit(0)
